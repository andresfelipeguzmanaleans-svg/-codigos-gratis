...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    const result: any = { type, ids: {}, meta: {} };
    ...
    if (type === 'game') {
      ...
      if (universeId) {
        ...
        if (game) {
          ...
          result.meta = {
            ...
            playing: game.playing || 0, visits: game.visits || 0,
            maxPlayers: game.maxPlayers || 0, created: game.created || '',
            updated: game.updated || '', genre: game.genre || '',
      ...
      result.meta = {
        name: userData.name || '', displayName: userData.displayName || '',
        description: userData.description || '', created: userData.created || '',
        isBanned: userData.isBanned || false, hasVerifiedBadge: userData.hasVerifiedBadge || false,
      ...
      result.meta = {
        ...
        created: detail.Created || '', updated: detail.Updated || '',
        priceInRobux: detail.PriceInRobux ?? null, sales: detail.Sales || 0,
        isForSale: detail.IsForSale || false, creator: detail.Creator?.Name || '',
        thumbnail: thumbData.data?.[0]?.imageUrl || null,
      };
    } else if (type === 'group') {
      ...
      result.meta = {
        ...
        memberCount: group.memberCount || 0, owner: group.owner?.username || '',
        isLocked: group.isLocked || false, thumbnail: null,
      };
      ...
      result.meta = {
        ...
        priceInRobux: gp.PriceInRobux ?? null, isForSale: gp.IsForSale || false,
        iconImageAssetId: gp.IconImageAssetId ? String(gp.IconImageAssetId) : null, thumbnail: null,
      };
      ...
      if (bundle.creator) result.ids.creatorId = String(bundle.creator.id || '');
      result.meta = {
        name: bundle.name || '', description: bundle.description || '',
        bundleType: bundle.bundleType || '', creator: bundle.creator?.name || '', thumbnail: null,
      };
      if (bundle.items && bundle.items.length > 0) {
        result.ids.itemIds = bundle.items.map((i: any) => String(i.id)).join(', ');
      }
    } else if (type === 'badge') {
